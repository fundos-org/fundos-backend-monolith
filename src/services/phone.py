...
COUNTRY_CODES = {
    ...
    "dubai": {"code": "971", "phone_length": 9},  # UAE code, as Dubai is a city
...
class PhoneService:
    ...
    async def send_phone_otp(
        ...
        try:
            ...
            if len(phone_number) != expected_length:
                raise HTTPException(status_code=400, detail=f"Phone number must be {expected_length} digits")
            
            # Add country code
            phone_with_country_code = f"{country_code}{phone_number}"
            full_phone_number = f"+{phone_with_country_code}"  # For response message
            ...
            params = {
                ...
                "mobile": phone_with_country_code,  # MSG91 expects number with country code
    ...
    async def verify_phone_otp(
        ...
        try:
            ...
            country_info = COUNTRY_CODES[country]
            country_code = country_info["code"]
            expected_phone_length = country_info["phone_length"]
            
            # Validate phone number: must be exactly the expected length and digits only
            if not phone_number.isdigit():
                raise HTTPException(status_code=400, detail="Phone number must contain only digits")
            if len(phone_number) != expected_phone_length:
                raise HTTPException(status_code=400, detail=f"Phone number must be {expected_phone_length} digits")
